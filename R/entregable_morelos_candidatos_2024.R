...
2
bd_encuestas_raw <- 
  openxlsx2::read_xlsx(file = dir_bd_gppolls, sheet = "Morelos", cols = seq.int(1:27)) |> 
...
bd_preparada <- 
  ...
  ungroup() |> 
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("Candidato MC", "OTRO", "Ninguno"),
                                    true = "Otro",
                                    false = candidato),
...
tabla_encuestas <- 
  ...
  janitor::clean_names() %>%
  arrange(fecha_fin) %>%
  mutate(diferencia = margarita_gonzalez - lucy_meza,
...
resultado_gppolls <- 
  ...
  mutate(across(where(is.numeric), ~ round(.x, digits = 0)),
         diferencia = margarita_gonzalez - lucy_meza,
         across(where(is.numeric), ~ scales::percent(.x/100, accuracy = 1.)),
         diferencia = gsub(pattern = "%", replacement = "", x = diferencia))
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_morelos_candidatos_%d_%B"), ".pptx", sep = "")
