...
bd_aprobacion_puebla <- openxlsx2::read_xlsx(file = "./Insumos/aprobracion_presidencial_puebla.xlsx") |> 
  as_tibble() |> 
  filter(!is.na(fecha)) |> 
...
g <- bd_aprobacion_puebla |> 
  ...
  scale_color_manual(values = c("aprobacion_amlo" = color_morena, "aprobacion_cespedes" = color_prd), name = "", 
                     labels = c("aprobacion_amlo" = "AMLO", "aprobacion_cespedes" = "Sergio Salomón Céspedes")) +
  theme(text = element_text(size = 24, family = "Poppins")) +
  labs(fill = "", color = "", title = "Aprobación")

ggsave("Entregable/aprobacion_puebla.png", g, scale = 2, width = 10, bg = "white", dpi = "retina")