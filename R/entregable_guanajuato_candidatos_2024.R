...
2
bd_encuestas_raw <- 
  openxlsx2::read_xlsx(file = dir_bd_gppolls, sheet = "Guanajuato", cols = seq.int(1:27)) |> 
...
bd_preparada <- 
  ...
  mutate(trackeable = dplyr::if_else(condition = all(c("Alma Alcaraz", "Libia García Muñoz") %in% candidato),
                                     true = T,
                                     false = F)) |> 
  ungroup() |> 
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("Desireé Ángel Rocha", "Yulma Rocha", "Gerardo Fernández", "Rodrigo González Zaragoza", "Virginia Magaña", "Ninguno",
                                                                 "Rodrigo  González Zaragoza", "Candidato Independiente"),
                                    true = "Otro",
                                    false = candidato),
         candidato = dplyr::if_else(condition = candidato %in% c("No sabe", "No sabe/No Respondió"),
                                    true = "Ns/Nc",
                                    false = candidato)) |> 
  mutate(colorHex = case_when(candidato == "Alma Alcaraz" ~ color_morena,
                              candidato == "Libia García Muñoz" ~ color_pan,
...
tabla_encuestas <- 
  ...
  janitor::clean_names() %>%
  arrange(fecha_fin) %>%
  mutate(diferencia = alma_alcaraz - libia_garcia_munoz,
  ...
  rename("Casa Encuestadora" = casa_encuestadora,
         "Alma Alcaraz" = alma_alcaraz,
         "Libia García\nMuñoz" = libia_garcia_munoz,
         "Diferencia\nventaja\n(puntos)" = diferencia,
         "Ns/Nc" = ns_nc,
         "Otro" = otro, 
         "Fecha de\ntérmino" = fecha_fin,
...
resultado_gppolls <- modelo_resultado[[1]] %>%
  ...
  mutate(across(where(is.numeric), ~ round(.x, digits = 0)),
         diferencia = alma_alcaraz - libia_garcia_munoz,
         across(where(is.numeric), ~ scales::percent(.x/100, accuracy = 1.)),
         diferencia = gsub(pattern = "%", replacement = "", x = diferencia))
...
modelo_resultado_elecciones <- 
  modelo_bayesiano(bd = bd_preparada %>% rename(partido = candidato), fechaFin = lubridate::as_date("2024-06-02"))
...
add_slide(pptx, layout = "1_portada", master = "Tema de Office") %>%
  ph_with(value = "ENCUESTAS GUANAJUATO 2024", location = ph_location_label(ph_label = "titulo")) %>%
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_guanajuato_candidatos_%d_%B"), ".pptx", sep = "")
