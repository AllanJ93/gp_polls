...
bd_preparada <- bd_encuestas_raw |>
  ...
  ungroup() |> 
  filter(trackeable == T) |>
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("MC", "Otro", "Otros", "Ninguno"),
                                    true = "Otro",
                                    false = candidato),
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_tuxtla_partidos_%d_%B"), ".pptx", sep = "")
