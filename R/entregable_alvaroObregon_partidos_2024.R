...
bd_preparada <- bd_encuestas_raw |> 
  ...
  select(id,
         ...
         candidato = partido_o_alianza, # DIFERENCIA ENTRE CANDIDATO O PARTIDO
         careo, 
         metodologia,
         calidad,
         numeroEntrevistas,
  ...
  ungroup() |>
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("MC", "Ninguno"),
                                    true = "Otro",
                                    false = candidato),
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_alvaroObregon_partidos_%d_%B"), ".pptx", sep = "")
