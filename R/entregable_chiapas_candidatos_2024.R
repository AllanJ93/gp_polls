...
2
bd_encuestas_raw <-
  openxlsx2::read_xlsx(file = dir_bd_gppolls, sheet = "Chiapas", cols = seq.int(1:27)) |> 
...
bd_preparada <-
  ...
  mutate(trackeable = dplyr::if_else(condition = all(c("Eduardo Ramírez Aguilar", "Willy Ochoa") %in% candidato),
                                     true = T,
                                     false = F)) |> 
  ungroup() |> 
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("Ninguno", "María Elena Orantes", "Candidato MC", "Otro", "Valeria Santiago Rojas",
                                                                 "Carlos Palomeque", "Rubén Zuarth", "Noé Castañón", 
                                                                 "Roberto Antonio Zuarth Esquinca", "Valeria Santiago", "Rita Guadalupe Balboa"),
                                    true = "Otro",
                                    false = candidato),
...
tabla_encuestas <- bd_preparada %>% 
  ...
  janitor::clean_names() %>%
  arrange(fecha_fin) %>% 
  mutate(diferencia = eduardo_ramirez_aguilar - willy_ochoa,
  ...
  rename("Casa Encuestadora" = casa_encuestadora,
         "Eduardo Ramírez\nAguilar" = eduardo_ramirez_aguilar,
         "Willy Ochoa" = willy_ochoa,
         "Diferencia\nventaja\n(puntos)" = diferencia,
         "Ns/Nc" = ns_nc,
         "Otro" = otro, 
         "Fecha de\ntérmino" = fecha_fin,
...
resultado_gppolls <- modelo_resultado[[1]] %>%
  ...
  mutate(across(where(is.numeric), ~ round(.x, digits = 0)),
         diferencia = eduardo_ramirez_aguilar - willy_ochoa,
         across(where(is.numeric), ~ scales::percent(.x/100, accuracy = 1.)),
         diferencia = gsub(pattern = "%", replacement = "", x = diferencia))
...
bd_triunfo <- 
  ...
  tidyr::complete(candidato = c("Eduardo Ramírez Aguilar", "Willy Ochoa"),
                  tidyr::nesting(fecha),
                  fill = list(color = color_pan,
                              prob = 0))
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_chiapas_candidatos_%d_%B"), ".pptx", sep = "")
