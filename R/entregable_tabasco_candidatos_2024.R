...
2
bd_encuestas_raw <- openxlsx2::read_xlsx(file = dir_bd_gppolls, sheet = "Tabasco", cols = seq.int(1:27)) |> 
...
bd_preparada <- bd_encuestas_raw |> 
  ...
  mutate(trackeable = dplyr::if_else(condition = all(c("Javier May Rodríguez", "Soraya Pérez", "Gerardo Guadiano") %in% candidato),
                                     true = T,
                                     false = F)) |> 
  ungroup() |> 
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("Otro"),
                                    true = "Otro",
                                    false = candidato),
         candidato = dplyr::if_else(condition = candidato %in% c("No sabe"),
                                    true = "Ns/Nc",
                                    false = candidato)) |> 
  mutate(colorHex = case_when(candidato == "Javier May Rodríguez" ~ color_morena,
                              candidato == "Soraya Pérez" ~ color_pan,
                              candidato == "Gerardo Guadiano" ~ color_mc,
...
tabla_encuestas <- 
  ...
  janitor::clean_names() %>%
  arrange(fecha_fin) %>%
  mutate(diferencia = javier_may_rodriguez - soraya_perez,
  ...
  rename("Casa Encuestadora" = casa_encuestadora,
         "Javier May\nRodriguez" = javier_may_rodriguez,
         "Soraya\nPérez" = soraya_perez,
         "Diferencia\nventaja\n(puntos)" = diferencia,
         "Gerardo\nGuadiano" = gerardo_guadiano,
         "Ns/Nc" = ns_nc,
         "Otro" = otro,
         "Fecha de\ntérmino" = fecha_fin,
...
resultado_gppolls <- 
  ...
  mutate(across(where(is.numeric), ~ round(.x, digits = 0)),
         diferencia = javier_may_rodriguez - soraya_perez,
         across(where(is.numeric), ~ scales::percent(.x/100, accuracy = 1.)),
         diferencia = gsub(pattern = "%", replacement = "", x = diferencia))
...
bd_triunfo <- 
  ...
  tidyr::complete(candidato = c("Javier May Rodríguez", "Soraya Pérez"),
                  fill = list(prob = 0),
                  tidyr::nesting(fecha)) |> 
  select(!color) |> 
  left_join(bd_puntos |> 
              distinct(candidato, color), by = "candidato")
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_tabasco_candidatos_%d_%B"), ".pptx", sep = "")
