...
2
bd_encuestas_raw <- 
  openxlsx2::read_xlsx(file = dir_bd_gppolls, sheet = "Yucatán", cols = seq.int(1:27)) |> 
...
bd_preparada <- 
  ...
  filter(lubridate::as_date("2024-01-01") <= fechaInicio) |> 
  select(id,
         casa_encuestadora,
         fechaInicio,
         fechaFin,
  ...
  mutate(trackeable = dplyr::if_else(condition = all(c("Joaquín ‘Huacho’ Díaz Mena", "Renán Barrera") %in% candidato),
                                     true = T,
                                     false = F)) |> 
  ungroup() |> 
  filter(trackeable == T) |>
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("Candidato MC", "Vida Gómez", "Ninguno", "Mauricio Sahui Rivero", 
                                                                 "Otro", "Luis Felipe Saidén", "Candidato PRD", "Tina Tuyub", 
                                                                 "Jazmín López Manrique", "Tina Tayub"),
                                    true = "Otro",
                                    false = candidato),
...
tabla_encuestas <- 
  ...
  janitor::clean_names() %>%
  arrange(fecha_fin) %>% 
  mutate(diferencia = joaquin_huacho_diaz_mena - renan_barrera,
  ...
  rename("Casa Encuestadora" = casa_encuestadora,
         "Joaquín ‘Huacho’\nDíaz Mena" = joaquin_huacho_diaz_mena,
         "Renán Barrera" = renan_barrera,
         "Diferencia\nventaja\n(puntos)" = diferencia,
         "Ns/Nc" = ns_nc,
         "Otro" = otro,
         "Fecha de\ntérmino" = fecha_fin,
...
resultado_gppolls <- 
  ...
  mutate(across(where(is.numeric), ~ round(.x, digits = 0)),
         diferencia = joaquin_huacho_diaz_mena - renan_barrera,
         across(where(is.numeric), ~ scales::percent(.x/100, accuracy = 1.)),
         diferencia = gsub(pattern = "%", replacement = "", x = diferencia))
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_yucatan_candidato_%d_%B"), ".pptx", sep = "")
