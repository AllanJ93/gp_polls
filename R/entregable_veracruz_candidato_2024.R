...
2
bd_encuestas_raw <- 
  openxlsx2::read_xlsx(file = dir_bd_gppolls, sheet = "Veracruz", cols = seq.int(1:27)) |> 
...
bd_preparada <- 
  ...
  ungroup() |> 
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("Candidato MC", "Otro", "José Manuel del Rio Virgen", "Sergio Gil",
                                                                 "Candidato", "José Manuel del Rio", "Hipólito Deschamps", 
                                                                 "Simón Sabino Galaviz", "No Votaría", "José Manuel del Río Virgen",
                                                                 "Ninguno"),
                                    true = "Otro",
                                    false = candidato),
