...
bd_aprobacion_yucatan <- openxlsx2::read_xlsx(file = "./Insumos/aprobracion_lideres_yucatan.xlsx") |> 
  as_tibble() |> 
  filter(!is.na(fecha)) |> 
...
g <- bd_aprobacion_yucatan |> 
  ...
  scale_color_manual(values = c("aprobacion_amlo" = color_morena, "aprobacion_vila" = color_pan), name = "", 
...
ggsave("Entregable/aprobacion_yucatan.png", g, scale = 2, width = 10, bg = "white", dpi = "retina")