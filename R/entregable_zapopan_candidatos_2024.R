...
2
bd_encuestas_raw <- 
  openxlsx2::read_xlsx(file = dir_bd_gppolls, sheet = "Zapopan", cols = seq.int(1:27)) |> 
...
bd_preparada <- 
  ...
  ungroup() |> mutate(candidato = dplyr::if_else(condition = grepl(pattern = "Oscar", x = candidato),
                                                 true = "Óscar Santos Rizo",
                                                 false = candidato)) |> 
  group_by(idIntencionVoto) |> 
  mutate(trackeable = dplyr::if_else(condition = all(c("Pedro Kumamoto", "Juan José Frangie") %in% candidato),
                                     true = T,
                                     false = F)) |> 
  ungroup() |> 
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("Omar Barboa", "Otro", "Óscar Santos Rizo",
                                                                 "Manuel Alfaro", "Ninguno", "No iré a votar",
                                                                 "Oscar Santos Rizo", "Voto nulo"),
                                    true = "Otro",
                                    false = candidato),
         candidato = dplyr::if_else(condition = candidato %in% c("No sabe"),
                                    true = "Ns/Nc",
                                    false = candidato)) |> 
  mutate(colorHex = case_when(candidato == "Pedro Kumamoto" ~ color_morena,
                              candidato == "Juan José Frangie" ~ color_mc,
                              candidato == "Otro" ~ color_pasnsnc,
...
tabla_encuestas <- bd_preparada %>% 
  ...
  janitor::clean_names() %>%
  arrange(fecha_fin) %>% 
  mutate(diferencia = pedro_kumamoto - juan_jose_frangie,
  ...
  rename("Casa Encuestadora" = casa_encuestadora,
         "Pedro Kumamoto" = pedro_kumamoto,
         "Juan José Frangie" = juan_jose_frangie,
         "Diferencia\nventaja\n(puntos)" = diferencia,
         "Ns/Nc" = ns_nc,
         "Otro" = otro,
         "Fecha de\ntérmino" = fecha_fin,
...
resultado_gppolls <- modelo_resultado[[1]] %>%
  ...
  mutate(across(where(is.numeric), ~ round(.x, digits = 0)),
         diferencia = pedro_kumamoto - juan_jose_frangie,
         across(where(is.numeric), ~ scales::percent(.x/100, accuracy = 1.)),
         diferencia = gsub(pattern = "%", replacement = "", x = diferencia))
...
add_slide(pptx, layout = "1_portada", master = "Tema de Office") %>%
  ph_with(value = "ENCUESTAS ZAPOPAN 2024", location = ph_location_label(ph_label = "titulo")) %>%
...
tabla_completa_anexos %>%
  purrr::walk( ~ add_slide(pptx, layout = "anexos_zapopan", master = "Tema de Office") %>%
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_zapopan_candidatos_%d_%B"), ".pptx", sep = "")
