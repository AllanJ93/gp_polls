...
id_bd_base <- "https://docs.google.com/spreadsheets/d/1M4ifUkX3ULaYoc0gdM2PDC6oEAjUPFAdikTU_jhePFs/edit#gid=1870197807"
...
2
bd_encuestas_raw <- openxlsx2::read_xlsx(file = dir_base, sheet = 4, cols = seq.int(1:27)) |> 
...
bd_preparada <- bd_encuestas_raw |>
  ...
  filter(!idIntencionVoto %in% c(12))
...
pptx_path <- paste("Entregable/GPPOLLS_guanajuato_", format(lubridate::today(), "%d%B"), ".pptx", sep = "")
