...
bd_preparada <- bd_encuestas_raw |>
  ...
  filter(id != "Ref_1") |> # SUMA MAS DE 100
  select(id,
         casa_encuestadora,
         fechaInicio,
         fechaFin,
  ...
  ungroup() |> 
  filter(trackeable == T) |> 
  mutate(candidato = dplyr::if_else(condition = candidato %in% c("HAGAMOS JALISCO", "FUTURO JALISCO", "OTRO"),
                                    true = "Otro",
                                    false = candidato),
  ...
  mutate(fechaInicio = dplyr::if_else(condition = dias_levantamiento == 0,
                                      ...
                                      false = fechaInicio)) |> 
  mutate(dias_levantamiento = as.numeric(fechaFin - fechaInicio)) %>%
  mutate(candidato = gsub(pattern = "_", replacement = "-", x = candidato)) |> 
  filter(!dias_levantamiento <= 0) |> 
  filter(!idIntencionVoto %in% c(25))
...
pptx_path <- paste(folder_path, format(lubridate::today(), "gppolls_jalisco_partidos_%d_%B"), ".pptx", sep = "")
